...
module.exports = {
    ...
    postBook: (title, quantity,author) => {
        let result = query(`
            insert into books (id,title,amount,author)
            values (null, ?, ?,?)
    ...
    getLoansUser: async (id) => {
        ...
        if (aux.length > 0) {
            let result = query(`
            select l.id as loanId, l.userId as userId, l.bookId as bookId, l.expiracyDate as expiracyDate, b.title as title, b.author as author
                from loans l
                join books b
                    on l.bookId = b.id
                where l.userId = ?
